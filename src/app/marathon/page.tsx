...
const marathon = () => {
  ...
  const fetchEvents = async () => {
    try {
      const res = await fetch("/api/marathon/getEvent");
  ...
  const handleDelete = async (id: string) => {
    const res = await fetch("/api/marathon/delEvent", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    ...
    <div className="relative min-h-screen bg-black">
      ...
      <div className="relative z-10 flex flex-col items-center justify-between min-h-screen">
        <header className="w-full bg-opacity-0 py-6">
          <h1 className="text-4xl text-white font-bold text-center">Sự kiện marathon </h1>
        </header>
